<!doctype html>
<html lang="en-US">
<head>
<title>Медали</title>
<meta charset="utf-8">
<meta name="description" content="Our Roadmap is a non-exhaustive list of the current goals we aim to achieve through new developments and continuous integration, made possible with the support of our generous patrons.">
<meta name="keywords" content="rpcs3, playstation, playstation 3, ps3, emulator, debugger, windows, linux, macos, freebsd, open source, nekotekina, kd11, roadmap">
<?php include 'lib/module/sys-meta.php';?>
<meta property="og:title" content="RPCS3 - Roadmap" />
<meta property="og:description" content="Our Roadmap is a non-exhaustive list of the current goals we aim to achieve through new developments and continuous integration, made possible with the support of our generous patrons." />
<meta property="og:image" content="https://rpcs3.net/img/meta/mobile/1200.png" />
<meta property="og:image:width" content="1200" />
<meta property="og:image:height" content="630" />
<meta property="og:url" content="https://rpcs3.net" />
<meta property="og:locale" content="en_US"/>
<meta property="og:type" content="website" />
<meta property="og:site_name" content="RPCS3" />

<meta name="twitter:title" content="RPCS3 - Roadmap">
<meta name="twitter:description" content="Our Roadmap is a non-exhaustive list of the current goals we aim to achieve through new developments and continuous integration, made possible with the support of our generous patrons.">
<meta name="twitter:image" content="https://rpcs3.net/img/meta/mobile/1200.png">
<meta name="twitter:site" content="@rpcs3">
<meta name="twitter:creator" content="@rpcs3">
<meta name="twitter:card" content="summary_large_image">
<?php include 'lib/module/sys-css.php';?>
<?php include 'lib/module/sys-js.php';?>
</head>
<body>
<?php include 'lib/module/sys-php.php';?>
<div class="page-con-content">
	<div class="banner-con-container darkmode-header">
		<div id="object-particles">
		</div>
		<div class="wavebar-con-container">
			<div class="wavebar-con-wrap">
				<div class="wavebar-svg-object">
				</div>
				<div class="wavebar-svg-object">
				</div>
			</div>
		</div>
		<div class="banner-con-backdrop darkmode-header">
		</div>
		<div class='banner-con-title fade-up-onstart'>
			<div class='banner-tx1-title fade-up-onstart pulsate'>
				<h1>Медали и Награды</h1>
			</div>
			<div class='banner-con-divider'>
			</div>
			<div class='banner-tx2-title fade-up-onstart'>
				<p>
					 Все возможные медали и награды за заслуги
				</p>
			</div>
		</div>
	</div>
	<div class="page-con-container">
		<div class="page-in-container">
			<div class="user-con-grouped">
				<div class='container-con-block darkmode-block'>
				<div class='container-con-wrapper'>
					<div class='container-tx1-block darkmode-txt'>
						<div class='container-emp-block2'>
						</div>
						<h2>Медали</h2>
					</div>
					<div class='container-tx2-block darkmode-txt'>
					</div>
				</div>
			</div>
					<div class="user-con-content darkmode-txt">
						<div class='user-img-avatar' style="background: url('/img/medals/1.png') no-repeat center; background-size: cover;">
						</div>
						<div class="user-con-wrapper">
							<div class='user-tx1-content darkmode-txt'>
								<span class="user-tx2-member ">
								Железный крест 3-й степени </span>
							</div>
							<div class='user-con-divider medal-bronz darkmode-divider'>
							</div>
							
							<div class="user-con-specsheet">
							<div class="user-con-specbox">
								<div class="specbox-con-part darkmode-specbox">
									<div class="specbox-tx1-part darkmode-txt">
										<span>10 Убийств</span>
									</div>
								</div>
								<div class="specbox-con-part darkmode-specbox">
									<div class="specbox-tx1-part darkmode-txt">
										<span>Уничтожить самолет</span>
									</div>
								</div>
								<div class="specbox-con-part darkmode-specbox">
									<div class="specbox-tx1-part darkmode-txt">
										<span>Уничтожения 3 танков</span>
									</div>
								</div>
							</div>
						  </div>
						</div>
					</div>
					<div class="user-con-content darkmode-txt">
						<div class='user-img-avatar' style="background: url('/img/medals/2.png') no-repeat center; background-size: cover;">
						</div>
						<div class="user-con-wrapper">
							<div class='user-tx1-content darkmode-txt'>
								<span class="user-tx2-member ">
								Железный крест 2-й степени </span>
							</div>
							<div class='user-con-divider medal-silver darkmode-divider'>
							</div>
							
							<div class="user-con-specsheet">
							<div class="user-con-specbox">
								<div class="specbox-con-part darkmode-specbox">
									<div class="specbox-tx1-part darkmode-txt">
										<span>20 Убийств</span>
									</div>
								</div>
								<div class="specbox-con-part darkmode-specbox">
									<div class="specbox-tx1-part darkmode-txt">
										<span>Уничтожения 3 самолетов</span>
									</div>
								</div>
								<div class="specbox-con-part darkmode-specbox">
									<div class="specbox-tx1-part darkmode-txt">
										<span>Уничтожения 8 танков</span>
									</div>
								</div>
							</div>
						  </div>
						</div>
					</div>
					<div class="user-con-content darkmode-txt">
						<div class='user-img-avatar' style="background: url('/img/medals/3.png') no-repeat center; background-size: cover;">
						</div>
						<div class="user-con-wrapper">
							<div class='user-tx1-content darkmode-txt'>
								<span class="user-tx2-member ">
								Железный крест 1-й степени </span>
							</div>
							<div class='user-con-divider medal-gold darkmode-divider'>
							</div>
							
							<div class="user-con-specsheet">
							<div class="user-con-specbox">
								<div class="specbox-con-part darkmode-specbox">
									<div class="specbox-tx1-part darkmode-txt">
										<span>30 Убийств</span>
									</div>
								</div>
								<div class="specbox-con-part darkmode-specbox">
									<div class="specbox-tx1-part darkmode-txt">
										<span>Уничтожения 5 самолетов</span>
									</div>
								</div>
								<div class="specbox-con-part darkmode-specbox">
									<div class="specbox-tx1-part darkmode-txt">
										<span>Уничтожения 15 танков</span>
									</div>
								</div>
							</div>
						  </div>
						</div>
					</div>
					<div class="user-con-content darkmode-txt">
						<div class='user-img-avatar' style="background: url('/img/medals/4.png') no-repeat center; background-size: cover;">
						</div>
						<div class="user-con-wrapper">
							<div class='user-tx1-content darkmode-txt'>
								<span class="user-tx2-member ">
								День Д </span>
							</div>
							<div class='user-con-divider medal-silver darkmode-divider'>
							</div>
							
							<div class="user-con-specsheet">
							<div class="user-con-specbox">
								<div class="specbox-con-part darkmode-specbox">
									<div class="specbox-tx1-part darkmode-txt">
										<span>Принять участие в успешной морской высадке</span>
									</div>
								</div>
							</div>
						  </div>
						</div>
					</div>
				</div>
		</div>
	</div>
</div>
<?php include 'lib/module/inc-footer.php';?>
</body>
</html>